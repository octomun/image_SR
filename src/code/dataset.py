...
class test_Dataset(object):
    def __init__(self, train_images_dir, patch_size, scale, use_fast_loader=False):
        self.image_files = sorted(glob.glob(train_images_dir + '/*'))
        self.patch_size = patch_size
        self.scale = scale
        self.use_fast_loader = use_fast_loader
    ...
    def __getitem__(self, idx):
        if self.use_fast_loader:
            lr = tf.io.read_file(self.image_files[idx])
            lr = tf.image.decode_jpeg(lr, channels=3)
            lr = pil_image.fromarray(lr.numpy())
        else:
            lr = pil_image.open(self.image_files[idx]).convert('RGB')

        lr = lr.resize((2048 // opt.scale, 2048 // opt.scale), pil_image.BICUBIC)

        lr = transforms.ToTensor()(lr).to(device) #.unsqueeze(0)
        return lr, self.image_files[idx]
